...
def deploy():
    ...
    _get_latest_version(source_folder)
...
def _create_directory_structure_if_necessary(site_folder):
    for subfolder in ('database', 'static', 'virualenv', 'source'):
        run('mkdir -p %s/%s' % (site_folder, subfolder))


def _get_latest_version(source_folder):
...
def _update_virtualenv(source_folder):
    ...
    if not exists(virtual_folder + '/bin/pip'):
        run('virtualenv --python=python %s' % (virtual_folder,))
